...
class Parser:
    ...
    def expression(self) -> ParserNode:
        ...
        while self.current_token != None and self.current_token.type in [TokenType.UNION]:
    ...
    def term(self) -> ParserNode:
        ...
        while self.current_token != None and self.current_token.type in [TokenType.CONCATENATION]:
    ...
    def symbol(self) -> ParserNode:
        ...
        if token.type == TokenType.LEFT_PARENTHESIS:
            ...
            if self.current_token == None or self.current_token.type != TokenType.RIGHT_PARENTHESIS:
    ...
    def unary_lookahead(self, context) -> ParserNode:
        if self.current_token is not None and self.current_token.type == TokenType.KLEENE_CLOSURE:
            self.forward()
            return KleeneClosureNode(context)
        elif self.current_token is not None and  self.current_token.type == TokenType.POSITIVE_CLOSURE:
            self.forward()
